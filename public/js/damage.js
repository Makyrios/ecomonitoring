function calculateDamageSum() {
  let DamageSum = document.getElementById('totalDamageAmount');
  let Damages = document.querySelectorAll("#pollutionTable tr:not(.hidden) td:nth-child(8)");
  let sum = 0;
  Damages.forEach(function (Damage) {
    if (Damage.textContent != '-') {
      sum += parseFloat(Damage.textContent);
    }
  });
  DamageSum.textContent = sum.toFixed(2);
}
...
function filterTable() {
    ...
    rows.forEach(function (row) {
        ...
        const year = row.querySelector("td:nth-child(7)").innerHTML;
